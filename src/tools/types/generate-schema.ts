...
/* eslint-disable */
import { Type, Types } from '${config.functions.pathToSharedFromSchema(
    'type-definitions'
  )}';
import { ${[...usedSpecialTypes].join(
    ', '
  )} } from '${config.functions.pathToSharedFromSchema('types/common')}';
import { NotPromise, checkValueAgainstType, computePropertiesOfType } from '${config.functions.pathToSharedFromSchema(
    'check-type'
  )}';
...
${[...usedSpecialTypes]
  ...
  const error = checkValueAgainstType(value, { kind: 'string' } as unknown as Type, schema as Types);
